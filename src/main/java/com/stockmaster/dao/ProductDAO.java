...
public class ProductDAO {
    ...
    public void insert(Product product) throws SQLException {
        String sql = "INSERT INTO products (name, description, category_id, supplier_id, price_buy, price_sell, stock_current, stock_min) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void update(Product product) throws SQLException {
        String sql = "UPDATE products SET name = ?, description = ?, category_id = ?, supplier_id = ?, price_buy = ?, price_sell = ?, stock_current = ?, stock_min = ? WHERE id = ?";
        try (Connection conn = DBManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, product.getName());
            pstmt.setString(2, product.getDescription());
